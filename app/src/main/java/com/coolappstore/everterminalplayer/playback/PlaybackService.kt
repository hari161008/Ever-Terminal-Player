package com.coolappstore.everterminalplayer.playback
...
import java.io.File
import com.coolappstore.everterminalplayer.MainActivity
...
import com.coolappstore.everterminalplayer.data.encodeCounts
...
import com.coolappstore.everterminalplayer.player.albumArtUri
...
class PlaybackService : MediaLibraryService() {
    ...
    companion object {
        ...
        val CMD_SLEEP_SET = SessionCommand("com.coolappstore.everterminalplayer.command.SLEEP_SET", Bundle.EMPTY)
        val CMD_SLEEP_GET = SessionCommand("com.coolappstore.everterminalplayer.command.SLEEP_GET", Bundle.EMPTY)
        val CMD_AUDIO_SESSION =
            SessionCommand("com.coolappstore.everterminalplayer.command.AUDIO_SESSION", Bundle.EMPTY)
        val CMD_TOGGLE_SHUFFLE =
            SessionCommand("com.coolappstore.everterminalplayer.command.TOGGLE_SHUFFLE", Bundle.EMPTY)
        val CMD_CYCLE_REPEAT =
            SessionCommand("com.coolappstore.everterminalplayer.command.CYCLE_REPEAT", Bundle.EMPTY)
        val CMD_SKIP_SILENCE =
            SessionCommand("com.coolappstore.everterminalplayer.command.SKIP_SILENCE", Bundle.EMPTY)
        val CMD_CLEAR_CACHE =
            SessionCommand("com.coolappstore.everterminalplayer.command.CLEAR_CACHE", Bundle.EMPTY)

        /** Shared on-disk cache for streamed (YT) audio so re-playing a track
         * doesn't re-download it. One instance per process, released only
         * when the app process dies. */
        @Volatile
        private var streamCache: SimpleCache? = null

        private fun streamCache(context: android.content.Context): SimpleCache =
            streamCache ?: synchronized(this) {
                streamCache ?: SimpleCache(
                    File(context.cacheDir, "dmt_stream_cache"),
    ...
    private inner class LibraryCallback : MediaLibrarySession.Callback {
        ...
        override fun onCustomCommand(
            ...
            CMD_SKIP_SILENCE.customAction -> {
                (session.player as? ExoPlayer)?.skipSilenceEnabled =
                    args.getBoolean(KEY_SKIP_SILENCE)
                Futures.immediateFuture(SessionResult(SessionResult.RESULT_SUCCESS))
            }

            CMD_CLEAR_CACHE.customAction -> {
                runCatching {
                    val cache = streamCache(this@PlaybackService)
