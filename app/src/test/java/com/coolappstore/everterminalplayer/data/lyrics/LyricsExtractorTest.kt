package com.coolappstore.everterminalplayer.data.lyrics
...
class LyricsExtractorTest {
    ...
    fun `falls back to UNSYNCEDLYRICS when nothing synced is present`() {
        ...
        assertEquals("[Intro]\nplain unsynced text", LyricsExtractor.extract(file.path, "audio/flac"))
    }
    ...
    fun `ranking is by content, not by which tag key holds it`() {
        ...
        assertEquals("<00:01.00>word <00:02.00>timed<00:03.00>", LyricsExtractor.extract(file.path, "audio/flac"))
    }
